...
class Brick:
    ...
    def make_brick_fall(self, fallen):
        if (self.start[Z] <= 1 or 
            any(map(lambda b: self.check_2d_collision(b),
    ...
    def safe_to_remove(self):
        return all(map(lambda o: len(o.supported_by) >= 2, self.supporting))
...
def main():
    ...
    print("Part 1:", len(list(filter(lambda b: b.safe_to_remove(), bricks))))

    print("Part 2:", sum(map(lambda b: b.chain_reaction_falls(), bricks)))
