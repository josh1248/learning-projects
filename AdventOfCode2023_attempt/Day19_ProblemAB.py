...
def main():
    ...
    p1_start = [[(int(i), int(i)) for i in re.findall(r'\d+', s)] for s in input[1].split("\n")]
    print(f"Part 1: {sum(map(lambda i_s: sum(i[0] for i in i_s) if get_accepted_parts(i_s, 'in', 0) else 0, p1_start)):,}")
