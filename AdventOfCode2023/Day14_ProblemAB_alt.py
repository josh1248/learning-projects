...
day_number = 14
def main():
    input = np.array([list(line.strip()) for line in read_file()]) #N-S column becomes W-E row
    def weight_on_north(grid):
        total = 0
        for i in range(1, len(grid) + 1):
            total += (grid[-i] == "O").sum() * i #numpy's version of .count()
        return total
        ...
        for r_index, row in enumerate(grid):
            ...
            for c_index, char in enumerate(row):
                if char == "O":
                    ...
                    grid[r_index, (c_index - rounded_rocks):c_index] = "O"
                    grid[r_index, (last_cube_rock + 1):(c_index - rounded_rocks)] = "."
                    last_cube_rock = c_index
                    rounded_rocks = 0
            if rounded_rocks:
                grid[r_index, -rounded_rocks:] = "O"
                grid[r_index, (last_cube_rock + 1):-rounded_rocks] = "."
